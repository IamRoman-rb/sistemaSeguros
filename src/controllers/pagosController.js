...
export const listar = async (req, res) => {
  ...
  export const recibo = async (req, res) => {
    ...
    try {
      let [pagos, clientes, polizas, usuarios, provincias, ciudades, coberturas, automarcas, sucursales]= await Promise.all(resources.map(async (file) => JSON.parse(await readFile(file, 'utf-8'))));
      ...
      pago.cliente.provincia = provincias.find(p => p.id === pago.cliente.provinciaId);
      pago.cliente.localidad = ciudades.find(c => c.id === pago.cliente.localidadId);
      pago.poliza.cobertura = coberturas.find(c => c.id === pago.poliza.coberturaId);
      pago.poliza.automarca = automarcas.find(a => a.id === pago.poliza.automarcaId);
      pago.poliza.sucursal = sucursales.find(s => s.id === pago.poliza.sucursalId);
      pago.cobrador.sucursal = sucursales.find(s => s.id === pago.cobrador.sucursalId);
  ...
  export const pagar = async (req, res) => {
    ...
    const [polizas, clientes, provincias, ciudades, coberturas, automarcas, pagos] = await Promise.all(resources.map(async (file) => JSON.parse(await readFile(file, 'utf-8'))));
...
export const acreditar = async (req, res) => {
  try {
    ...
    polizas[polizas.findIndex((p) => p.id === Number(polizaId))].pagos.push(nuevoIdPago);
