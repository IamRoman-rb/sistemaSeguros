...
export const caja = async (req, res) => {
  ...
  try {
    ...
    const diaActual = ahoraArgentina.day;
      ...
      let [pagos, polizas, caja, sucursales, coberturas, clientes, usuarios] = await Promise.all(
      ...
      ingresos = ingresos.filter(ingreso => {
        const fechaIngreso = DateTime.fromISO(ingreso.fecha).setZone('America/Argentina/Buenos_Aires');
        return fechaIngreso.year === anioActual && fechaIngreso.month === mesActual && fechaIngreso.day === diaActual;
    });

    egresos = egresos.filter(egreso => {
      const fechaEgreso = DateTime.fromISO(egreso.fecha).setZone('America/Argentina/Buenos_Aires');
      return fechaEgreso.year === anioActual && fechaEgreso.month === mesActual && fechaEgreso.day === diaActual;
  });

  pagos = pagos.filter(pago => {
    const fechaPago = DateTime.fromISO(pago.fecha).setZone('America/Argentina/Buenos_Aires');
    return pago.id_cobrador == usuario_filtrado.id && fechaPago.year === anioActual && fechaPago.month === mesActual && fechaPago.day === diaActual && !pago.desconocido;
});

pagos = pagos.map(pago => {
  const poliza = polizas.find(poliza => poliza.id === Number(pago.id_poliza));
  const cliente = clientes.find(cliente => cliente.id === Number(poliza?.clienteId));
  const cobrador = usuarios.find(usuarios => usuarios.id === Number(pago.id_cobrador));
  const sucursal = sucursales.find(sucursal => sucursal.id === Number(cobrador?.sucursal));
  const cobertura = coberturas.find(cobertura => cobertura.id === Number(poliza?.cobertura));

  return {
      fecha: pago.fecha,
      valor: pago.valor,
      forma_pago: pago.forma_pago,
      cliente: cliente ? `${cliente.nombre} | ${cliente.cuit}` : 'Cliente no encontrado',
      cobrador: cobrador && sucursal ? `${cobrador.nombre} | ${sucursal.nombre}` : 'Cobrador o sucursal no encontrado',
      descripcion: poliza && cobertura ? `Poliza N° ${poliza.n_poliza} | ${cobertura.nombre}` : 'Póliza o cobertura no encontrada',
  };
});

      let pagosEnEfectivo = pagos.filter((pago) => pago.forma_pago === "efectivo");

      let total = pagosEnEfectivo.reduce((p, a) => p + Number(a.valor), 0) + ingresos.reduce((p, a) => p + Number(a.monto), 0) - egresos.reduce((p, a) => p + Number(a.monto), 0);      
...
export const resumen = async (req, res) => {
  ...
  try {
    ...
    // Mapear cobradores
    caja = caja.map((c) => ({ ...c, cobrador: usuarios.find((u) => u.id === Number(c.id_usuario)) }));
    pagos = pagos.map((p) => ({ ...p, cobrador: usuarios.find((u) => u.id === Number(p.id_cobrador)) }));

    // Filtrar por sucursal
    caja = caja.filter((c) => c.cobrador && c.cobrador.sucursal == usuario_filtrado.sucursal);
    pagos = pagos.filter((p) => p.cobrador && p.cobrador.sucursal == usuario_filtrado.sucursal);

    resumenes = resumenes.map((resumen) => {
      const fechaInicioMes = DateTime.fromObject({ year: resumen.anio, month: resumen.mes }).setZone('America/Argentina/Buenos_Aires');
      const fechaFinMes = fechaInicioMes.endOf('month');

      let ingresosCaja = caja.filter(item =>
        item.tipo === "ingreso" &&
        DateTime.fromISO(item.fecha).setZone('America/Argentina/Buenos_Aires') >= fechaInicioMes &&
        DateTime.fromISO(item.fecha).setZone('America/Argentina/Buenos_Aires') <= fechaFinMes
      ).map(item => Number(item.monto));

      let egresosCaja = caja.filter(item =>
        item.tipo === "egreso" &&
        DateTime.fromISO(item.fecha).setZone('America/Argentina/Buenos_Aires') >= fechaInicioMes &&
        DateTime.fromISO(item.fecha).setZone('America/Argentina/Buenos_Aires') <= fechaFinMes
      ).map(item => Number(item.monto));

      let ingresosPagos = pagos.filter(item =>
...
export const detalle = async (req, res) => {
  ...
  try {
    ...
    const fechaParametro = DateTime.fromMillis(Number(req.params.id)).setZone('America/Argentina/Buenos_Aires');
    const month = fechaParametro.month;
    const year = fechaParametro.year;
    const countDays = fechaParametro.daysInMonth;

    caja = caja.filter(c => DateTime.fromISO(c.fecha).setZone('America/Argentina/Buenos_Aires').month === month && DateTime.fromISO(c.fecha).setZone('America/Argentina/Buenos_Aires').year === year);
    pagos = pagos.filter(p => DateTime.fromISO(p.fecha).setZone('America/Argentina/Buenos_Aires').month === month && DateTime.fromISO(p.fecha).setZone('America/Argentina/Buenos_Aires').year === year);


    let resumen = Array.from({ length: countDays }, (_, i) => {
      const date = DateTime.fromObject({ year, month, day: i + 1 }).setZone('America/Argentina/Buenos_Aires');

      let ingresosCaja = caja.filter(item =>
        item.tipo === "ingreso" &&
        DateTime.fromISO(item.fecha).setZone('America/Argentina/Buenos_Aires').day === date.day
      ).map(item => Number(item.monto));
      ...
      let egresosPagos = pagos.filter(item =>
