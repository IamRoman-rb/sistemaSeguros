...
export const detalle = async (req, res) => {
  ...
  try {
    let [pagos, clientes, polizas, usuarios, provincias, ciudades, sucursales, empresas]= await Promise.all(resources.map(async (file) => JSON.parse(await readFile(file, 'utf-8'))));

    pagos = pagos.map(pago => ({
      ...pago,
      cliente: clientes.find(c => c.id === Number(pago.id_cliente)) || { nombre: 'Cliente no encontrado' },
      poliza: polizas.find(p => p.id === Number(pago.id_poliza)) || { n_poliza: 'Póliza no encontrada' },
...
export const pagar = async (req, res) => {
  ...
  const [polizas, clientes, provincias, ciudades, pagos, empresas] = await Promise.all(resources.map(async (file) => JSON.parse(await readFile(file, 'utf-8'))));
